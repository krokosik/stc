...
class ApiUploader:
    ...
    async def guess_external_id(self, file_path: str) -> str | None:
        ...
        if base_name.startswith("1-s2.0"):
            ...
            async with self._session as session:
                crossref_response = await session.get(
                    f"https://api.crossref.org/works/?filter=alternative-id:{alternative_id}"
                )
    ...
    async def upload_file(
        self, file_path_or_data: str | bytes, external_id: str = None
    ):
        ...
        form_data.add_field(
            "file",
            (
                open(file_path_or_data, "rb")
                if isinstance(file_path_or_data, str)
                else file_path_or_data
            ),
